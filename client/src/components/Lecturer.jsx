import Badge from "./Badge"
import {Link} from "react-router-dom"
import React from "react"
export default function Student({name, lecturerId, courses,profilePhoto}) {
    return (
       <Link to={`/view-lecturers/${lecturerId}`}>
           <div className="w-60 flex flex-col items-center justify-center transition-transform duration-200 ease-in-out hover:scale-105 cursor-pointer">
                {/* Student Card */}
                <div className="relative bg-[#ffffff] rounded-3xl px-5 pt-4 pb-8 shadow-lg">
                    {/* Blue corner accents */}
                    <div className="absolute top-0 left-0 w-12 h-12 bg-[#253d90] rounded-tl-3xl rounded-br-3xl"></div>
                    <div className="absolute bottom-0 right-0 w-12 h-12 bg-[#253d90] rounded-br-3xl rounded-tl-3xl"></div>
                    <div className="flex flex-col items-center text-center space-y-2">
                        {/* Profile Image */}
                        <div className="w-24 h-24 rounded-full border-4 border-[#ffc20e] overflow-hidden">
                            <img
                            src={`http://localhost:8000${profilePhoto}`}
                            alt={name}
                            className="w-full h-full object-cover"
                            />
                        </div>
                        {/* Name */}
                        <h2 className="text-2xl font-bold text-[#000000]">{name}</h2>
                        {/* Student Details */}
                        <div className="space-y-2 text-[#000000]">
                            <p className="text-lg font-medium">Student ID: {lecturerId}</p>
                            <div>
                                <p className="font-medium">Subjects Taught:</p>
                                <p className="text-gray-700">Physics,Chemistry</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
       </Link>

    )
}